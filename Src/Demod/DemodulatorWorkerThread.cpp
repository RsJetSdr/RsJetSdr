...
void DemodulatorWorkerThread::run() {
    ...
    std::cout << "DemodulatorWorkerThread::thread started.." << std::endl;
    ...
    while (!stopping) {
        ...
        if ((makeDemod || filterChanged) && !stopping) {
            ...
            if (makeDemod) {
				...
				Message("wxGetApp().getAppFrame()->notifyUpdateModemProperties()")
            ...
            if (cModem && result.sampleRate && result.bandwidth) {
                ...
                result.iqResampleRatio = (float) (result.bandwidth) / (float) result.sampleRate;
   ...
   std::cout << "DemodulatorWorkerThread::thread done....." << std::endl;
}
