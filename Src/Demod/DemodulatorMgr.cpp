...
void DemodulatorMgr::deleteThread(DemodulatorInstancePtr demod) {
    ...
    std::lock_guard < std::recursive_mutex > lock(demods_busy);
	Message("Check wxGetApp().getBookmarkMgr().addRecent(demod)")
    ...
    //Ask for termination
    demod->setActive(false); //audio thread
    demod->terminate();
}
...
std::vector<DemodulatorInstancePtr> DemodulatorMgr::getDemodulatorsAt(long long freq, int bandwidth) {
    ...
    for (size_t i = 0, iMax = demods.size(); i < iMax; i++) {
...
#endif
#pragma message (__FILE__ "[" STRING(__LINE__) "]: wxGetApp()") 
		Message("wxGetApp().getBookmarkMgr().updateActiveList()")
    ...
    if (demod) {
        demod->setVisualOutputQueue(rsGetApp().getAudioVisualQueue());
 
        activeVisualDemodulator = demod;
    } else {
        DemodulatorInstancePtr last = getLastActiveDemodulator();
        if (last) {
            last->setVisualOutputQueue(rsGetApp().getAudioVisualQueue());
        }
...
void DemodulatorMgr::setLastBandwidth(int lastBandwidth) {
    if (lastBandwidth < MIN_BANDWIDTH) {
		...
		if (lastBandwidth > rsGetApp().getSampleRate())
		{
        lastBandwidth = rsGetApp().getSampleRate();
       }
