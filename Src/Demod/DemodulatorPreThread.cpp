...
//    std::cout << "Demodulator preprocessor thread started.." << std::endl;
    ...
    while (!stopping) {
        ...
        if ((currentFrequency - inp->frequency) != shiftFrequency) {
            shiftFrequency = (int)(currentFrequency - inp->frequency);
            if (abs(shiftFrequency) <= (int) ((double) (inp->sampleRate / 2) * 1.5)) {
                nco_crcf_set_frequency(freqShifter, (2.0f * M_PI) * (((float) abs(shiftFrequency)) / ((float) inp->sampleRate)));
            }
        }
...
//        std::lock_guard < std::mutex > lock(inp->m_mutex);
        ...
        if (data->size() && (inp->sampleRate == currentSampleRate) && cModem && cModemKit) {
			...
			unsigned int bufSize = (unsigned int)data->size();
            ...
            size_t out_size = (size_t)ceil((double) (bufSize) * iqResampleRatio) + 512;
