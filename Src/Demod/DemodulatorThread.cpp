...
//    std::cout << "Demodulator thread started.." << std::endl;
    ...
    while (!stopping) {
        ...
        if (modemAnalog != nullptr) {
            ...
            ati->sampleRate = (int)cModemKit->sampleRate;
            ati->inputRate = inp->sampleRate;
            ati->data.resize(0);
        }
        ...
        if (audioOutputQueue != nullptr && ati && ati->data.size()) {
            double accum = 0;
             ...
             if (cModem->useSignalOutput()) {
                ...
                currentSignalLevel = (float)linearToDb(accum / double(ati->data.size()));
                ...
                currentSignalLevel = (float)linearToDb(accum / double(inp->data.size()));
            }
        ...
        if (currentSignalLevel > signalLevel) {
            signalLevel = signalLevel + (currentSignalLevel - signalLevel) * 0.5f;
        } else {
            signalLevel = signalLevel + (currentSignalLevel - signalLevel) * 0.05f * sampleTime * 30.0f;
        }
        ...
        if (squelchEnabled) {
            if (!squelched && !squelchBreak) 
			{
				...
				if (rsGetApp().getSoloMode() /* && !wxGetApp().getAppFrame()->isUserDemodBusy()*/) 
					{
                        std::lock_guard < std::mutex > lock(squelchLockMutex);
                        if (squelchLock == nullptr) {
                            squelchLock = demodInstance;
                            rsGetApp().getDemodMgr().setActiveDemodulator(nullptr);
                            rsGetApp().getDemodMgr().setActiveDemodulatorByRawPointer(demodInstance, false);
                            squelchBreak = true;
        ...
        if (!squelched && (ati || modemDigital) && localAudioVisOutputQueue != nullptr && localAudioVisOutputQueue->empty()) {
            ...
            if (modemDigital) {
                ...
                ati_vis->channels = 2;
                for (int i = 0, iMax = (int)inputData->size() / 2; i < iMax; i++) {
                ...
                ati_vis->channels = 2;
                int stereoSize = (int)ati->data.size();
                ...
                if (inp->modemName == "I/Q") {
                    for (int i = 0; i < stereoSize / 2; i++) {
                        ati_vis->data[i] = (*inputData)[i].real * 0.75f;
                        ati_vis->data[i + stereoSize / 2] = (*inputData)[i].imag * 0.75f;
                    }
                } else {
                    for (int i = 0; i < stereoSize / 2; i++) {
        ...
        if (!squelched && ati != nullptr) {
            if (!muted.load() && (!rsGetApp().getSoloMode() || (demodInstance == rsGetApp().getDemodMgr().getLastActiveDemodulator().get()))) {
