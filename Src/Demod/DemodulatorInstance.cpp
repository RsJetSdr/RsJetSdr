...
#define MAX_WAIT_FOR_TERMINATION_MS (3000.0f)
    ...
    while (currentCycle < nbCyclesToWait) {
        ...
        if (isTerminated()) {
            std::cout << "DemodulatorInstance::Garbage collected demodulator instance '" << getLabel() << "'... " << std::endl << std::flush;
...
void DemodulatorInstance::updateLabel(long long freq) {
	...
	Message("Check wxGetApp().getBookmarkMgr().updateActiveList();")
...
void DemodulatorInstance::terminate() {
  ...
  //  std::cout << "DemodulatorInstance::Terminating demodulator audio thread.." << std::endl;
    audioThread->terminate();

 //   std::cout << "DemodulatorInstance::Terminating demodulator thread.." << std::endl;
    demodulatorThread->terminate();
   
  //  std::cout << "DemodulatorInstance::Terminating demodulator preprocessor thread.." << std::endl;
...
void DemodulatorInstance::setSquelchLevel(float signal_level_in) {
    ...
    rsGetApp().getDemodMgr().setLastSquelchLevel(signal_level_in);

   rsGetApp().getDemodMgr().setLastSquelchEnabled(true);
}
...
void DemodulatorInstance::setMuted(bool muted) {
    ...
    rsGetApp().getDemodMgr().setLastMuted(muted);
}
...
Message("Check DemodulatorInstance::startRecording()")
/*
