...
SDRPostThread::SDRPostThread() : IOThread(),
	buffers("SDRPostThreadBuffers"),
	visualDataBuffers("SDRPostThreadVisualDataBuffers"),
	frequency(0) {
    ...
    // Channel mode default is PFBCH (polyphase filterbank channelizer)
...
void SDRPostThread::updateActiveDemodulators() {
    ...
    long long centerFreq = rsGetApp().getFrequency();
	//retreive the current list of demodulators:
	
	auto demodulators = rsGetApp().getDemodMgr().getDemodulators();
	for (auto demod : demodulators) {
      // not in range?
        ...
        if (abs(frequency - demod->getFrequency()) > (sampleRate / 2)) {
            ...
            if (rsGetApp().getDemodMgr().getLastActiveDemodulator() == demod) {
            ...
            if (demod->isFollow() && centerFreq != demod->getFrequency()) {
                rsGetApp().setFrequency(demod->getFrequency());
                demod->setFollow(false);
            }
        } else if (!demod->isActive()) { // in range, activate if not activated
            demod->setActive(true);
            if (rsGetApp().getDemodMgr().getLastActiveDemodulator() == nullptr) {

                rsGetApp().getDemodMgr().setActiveDemodulator(demod);
            }
        }
...
//Поиск канала для заданной частоты
...
#endif
	std::cout << std::endl;
    std::cout << "SDRPostThread::thread started....." << std::endl;
   // Connect Queue
    ...
    while (!stopping) {
        ...
        if (data_in && data_in->data.size()) {
            ...
            if(data_in->numChannels > 1) {
                if (chanMode == 1) {
                    runPFBCH(data_in.get());  //firpfbch_crcf_analyzer_execute
                } else if (chanMode == 2) {
                    runPFBCH2(data_in.get()); //firpfbch2_crcf_execute
                }
            } else {
	...
	if (iqDataInQueue) 	iqDataInQueue->flush();
	if (iqDataOutQueue) 	iqDataOutQueue->flush();
	if (iqActiveDemodVisualQueue)  iqActiveDemodVisualQueue->flush();
	std::cout << "SDRPostThread::thread done....." << std::endl;
}
...
void SDRPostThread::runDemodChannels(int channelBandwidth) {
    ...
    DemodulatorInstancePtr activeDemod = rsGetApp().getDemodMgr().getLastActiveDemodulator();
...
void SDRPostThread::initPFBCH() {
	...
	std::cout << "SDRPostThread::Init PFCB with " << numChannels << " channels.";
